...
int main(int argc, char const *argv[]) {
  ...
  // publisher<bool> power_request("ACS_POWER_REQUEST", argv[1]);
  // subscriber<bool> power_reply("ACS_POWER_REPLY", argv[1]);

  // subscriber<bool> spin_stab_sub("SPIN_STAB", argv[1]);
  // subscriber<bool> sun_point_sub("SUN_POINT", argv[1]);
  // subscriber<bool> dark_side_sub("DARK_SIDE", argv[1]);

  // subscriber<ACS_DATA_BLOCK> acs_sub("ACS_DATA", argv[1]);
